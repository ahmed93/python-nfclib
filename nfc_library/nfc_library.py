...
class NFCLibrary:
        ...
        for field in required_fields:
            if field not in config:
                if field == "devices":
                    ...
                    config[field] = 5.0
                self.logger.warning(
                    f"Missing required field '{field}' in config. Using default value: {config[field]}"
                )
    ...
    def _find_device_path(self, device_id: str) -> Optional[str]:
        for device in self.config["devices"]:
            if device["id"] == device_id:
                if self.logger.isEnabledFor(logging.DEBUG):
                    self.logger.debug(
                        f"Found path for device {device_id}: {device['path']}"
                    )
                return device["path"]
    ...
    def _ensure_device_active(self, device_id: str) -> bool:
        ...
        if (
            ...
            and self.active_clf is not None
        ...
        try:
            self.logger.info(
                f"Opening connection to device {device_id} at {device_path}"
            )
            ...
            if not result["success"]:
                if thread.is_alive():
                    ...
                    self.logger.error(
                        f"Error opening device {device_id}: {result['error']}"
                    )
                    return False
    ...
    def _create_ndef_message(self, message: Union[str, Dict, List]) -> Any:
        ...
        if isinstance(message, str):
            ...
            if "type" not in message or "value" not in message:
                self.logger.error(
                    "Dictionary message missing required 'type' or 'value' keys"
                )
            ...
            for i, record in enumerate(message):
                if (
                    ...
                    raise ValueError(
                        "Each record in the list must be a dictionary with 'type' and 'value' keys"
                    )

                if record["type"] == "text":
                    if self.logger.isEnabledFor(logging.DEBUG):
                        self.logger.debug(
                            f"Creating text record {i}: {record['value']}"
                        )
                    ...
                    self.logger.error(
                        f"Unsupported record type in record {i}: {record['type']}"
                    )
    ...
    def _validate_write(self, tag, ndef_message: List) -> bool:
        ...
        if len(original_records) != len(written_records):
            self.logger.error(
                f"Validation failed: Record count mismatch - original: {len(original_records)}, written: {len(written_records)}"
            )
            return False
...
def create_nfc_library(
    config_path: str = "config.json", logger: Optional[logging.Logger] = None
) -> NFCLibrary:
